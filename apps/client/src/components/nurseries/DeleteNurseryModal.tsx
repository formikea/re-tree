...
interface DeleteNurseryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export function DeleteNurseryModal({ 
  isOpen, 
  onClose, 
  onConfirm, 
  nursery, 
  isLoading 
}: DeleteNurseryModalProps) {
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        <div className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Delete Nursery</h2>
              <p className="text-slate-600">This action cannot be undone.</p>
            </div>
          </div>
          
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <p className="text-red-800 font-medium mb-2">Are you sure you want to delete this nursery?</p>
            <div className="text-red-700 text-sm space-y-1">
              <p><span className="font-medium">Name:</span> {nursery.name}</p>
              <p><span className="font-medium">ID:</span> {nursery.id}</p>
            </div>
          </div>
