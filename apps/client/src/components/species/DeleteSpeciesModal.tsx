...
interface DeleteSpeciesModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  species: ApiSpecies | null;
  isLoading: boolean;
}

export function DeleteSpeciesModal({ 
  isOpen, 
  onClose, 
  onConfirm, 
  species, 
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        <div className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Delete Species</h2>
              <p className="text-slate-600">This action cannot be undone.</p>
            </div>
          </div>
          
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <p className="text-red-800 font-medium mb-2">Are you sure you want to delete this species?</p>
            <div className="text-red-700 text-sm space-y-1">
              <p><span className="font-medium">Botanical Name:</span> {species.botanicalName}</p>
              <p><span className="font-medium">Common Name:</span> {species.commonName}</p>
              {species.maoriName && (
                <p><span className="font-medium">Māori Name:</span> {species.maoriName}</p>
              )}
            </div>
          </div>
